...
if docker volume ls --format '{{.Name}}' | grep -q "^${PG_VOLUME_NAME}$"; then
  echo "SUPPRESSION volume: $PG_VOLUME_NAME"
  docker volume rm "$PG_VOLUME_NAME" >/dev/null
else
  echo "INFO: volume absent: $PG_VOLUME_NAME"
fi

# 4) Redeploy Postgres
echo "DEPLOY Postgres: $PG_STACK_NAME"
docker stack deploy -c "$POSTGRES_YML_PATH" "$PG_STACK_NAME" >/dev/null

# 5) Wait ready
CID="$(wait_postgres_ready 240)" || { echo "ERREUR: Postgres non prêt"; exit 1; }

# 6) Restore (stream gzip -> psql)
echo "RESTORE en cours (pg_dumpall -> psql)..."
gzip -dc "$BACKUP_PATH_HOST" | docker exec -i "$CID" sh -c '
  export PGPASSWORD="$(cat /run/secrets/pg_password)";
  psql -U postgres -v ON_ERROR_STOP=1
'

echo "OK: restore cluster terminé."
echo "INFO: vous pouvez maintenant redéployer les stacks applicatives (Keycloak, etc.)."
